...
for kt in np.arange(12):
	...
	temp_south.interpolate_from(woadir + woatemp,'temp',frame=kt,depthname='st_ocean', \
	use_locstream=True,from_global=False)
	temp_north.interpolate_from(woadir + woatemp,'temp',frame=kt,depthname='st_ocean', \
	use_locstream=True,from_global=False)
	temp_west.interpolate_from( woadir + woatemp,'temp',frame=kt,depthname='st_ocean', \
	use_locstream=True,from_global=False)

	salt_south.interpolate_from(woadir + woasalt,'salt',frame=kt,depthname='st_ocean', \
	use_locstream=True,from_global=False)
	salt_north.interpolate_from(woadir + woasalt,'salt',frame=kt,depthname='st_ocean', \
	use_locstream=True,from_global=False)
	salt_west.interpolate_from( woadir + woasalt,'salt',frame=kt,depthname='st_ocean', \
	use_locstream=True,from_global=False)
